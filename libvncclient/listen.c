...
listenForIncomingConnectionsNoFork(rfbClient* client, int timeout) {
...
#ifdef LIBVNCSERVER_IPv6 /* only try that if we're IPv6-capable, otherwise we may try to bind to the same port which would make all that listening fail */ 
    /* only do IPv6 listen of listen6Port is set */
    if (client->listen6Port > 0 && client->listen6Sock == INVALID_SOCKET) {
...
#endif
    if (client->listenSock == INVALID_SOCKET && client->listen6Sock == INVALID_SOCKET)
        return -1;
    ...
    /* Be sure to close the sockets used for listening */
    if (client->listenSock != INVALID_SOCKET) {
    ...
    /* r is now either 0 (timeout) or -1 (error) or 1 (success) */
    return r > 0?(client->sock != INVALID_SOCKET?1:-1):r;
}
